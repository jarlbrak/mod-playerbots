...
- **VRAM measurement**: two `rocm-smi --showmeminfo vram --json` samples per (model × slot) cell — one immediately after `/health` 200 (idle/load-only footprint) and one mid-steady-state at the 60 s mark (load + KV cache under traffic). `summary.md` records both as `vram_idle_mb` and `vram_loaded_mb`. `rocm-smi` works for VRAM reporting even when the runtime is Vulkan.
...
5. Sample `rocm-smi --showmeminfo vram --json` twice per (model × slot) cell: once immediately after `/health` 200 (`vram_idle_mb`) and once at the 60 s mark of steady-state (`vram_loaded_mb`). Capture `vram_used_mb`, `vram_total_mb` from each. Grammar form does not affect VRAM, so two samples per (model × slot) — not per (model × grammar × slot) — is enough.
